"""
Random Forest Model for Affective State Recognition
Uses engineered features for multi-task classification.

Features:
- Multi-task learning (4 affective states)
- Class-balanced training with SMOTE and class weights
...
class EngagementRandomForest:
    """
    Random Forest model for affective state recognition.

    Multi-task learning approach:
    - Trains separate RandomForest classifier for each affective state
    ...
    def __init__(
        ...
        **rf_params,
        ...
        self.stage1_models: Dict[str, Optional[RandomForestClassifier]] = {
            state: None for state in self.AFFECTIVE_STATES
        }
        self.stage2_models: Dict[str, Optional[RandomForestClassifier]] = {
            state: None for state in self.AFFECTIVE_STATES
        }
    ...
    def _train_two_stage(
        ...
        if self.use_smote and minority_ratio_s1 < 0.3:
            ...
            X_train_s1, y_train_s1 = self._apply_smote(
                X_train_s1, y_train_s1, random_state=params.get("random_state", 42)
            )
            if verbose:
                s1_dist_after = dict(zip(*np.unique(y_train_s1, return_counts=True)))
                print(f"    Stage 1 after SMOTE: {s1_dist_after}")

        stage1_model = RandomForestClassifier(**params, class_weight="balanced")
        ...
        if len(np.unique(y_train_stage2)) < 2:
            ...
            stage2_model = RandomForestClassifier(**params, class_weight="balanced")
    ...
    def fit(
        ...
        for state in self.AFFECTIVE_STATES:
            ...
            if use_two_stage_for_state and self.num_classes == 3:
                ...
                model.fit(X_train_state, y_train_state, sample_weight=sample_weights)
            ...
            if (
                ...
                try:
                    ...
                    calib_model = RandomForestClassifier(**params)
            ...
            if verbose:
                if model is not None:
                    print(f"  Training complete - {params.get('n_estimators', 300)} trees")
                else:
    ...
    @classmethod
    def load(cls, filepath: str) -> "EngagementRandomForest":
...
if __name__ == "__main__":
    ...
    model.save("test_random_forest.pkl")

    loaded_model = EngagementRandomForest.load("test_random_forest.pkl")
