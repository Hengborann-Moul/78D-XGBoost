...
Table~\ref{tab:importance-correlation} presents the correlation coefficients between SHAP and permutation importance rankings for each affective state.
...
\begin{itemize}
    \item \textbf{SHAP values} capture how features influence predictions \textit{on average} across the dataset, accounting for complex feature interactions.
    \item \textbf{Permutation importance} measures the \textit{marginal} contribution of each feature when its values are randomized, reflecting ensemble robustness rather than predictive power.
\end{itemize}

This discrepancy suggests that the most predictive features (high SHAP) may be highly correlated with other features, so shuffling them has limited impact on overall performance (low permutation importance). Conversely, features with high permutation importance may capture unique variance not well-captured by the model's learned interactions.

\subsection{Feature Importance in Engineered Feature Space}
\label{sec:engineered-feature-importance}

Beyond the original 78 MediaPipe features, we also analyzed importance in the expanded 2,152-dimensional engineered feature space. Table~\ref{tab:top-engineered-features} presents the top 10 features by consensus ranking across XGBoost gain, SHAP, and permutation importance.
...
\begin{enumerate}
    \item \textbf{Composite indicators:} Features like \texttt{confusion\_detailed} (combining browDown, eyeSquint, mouthFrown) and \texttt{engagement\_detailed} rank highest, validating the domain-aware feature engineering approach.

    \item \textbf{Spectral features:} Dominant frequency features (derived from FFT analysis) account for 40\% of top 10 features, indicating that temporal dynamics encoded in frequency domain are highly discriminative.

    \item \textbf{Dynamic features:} Features capturing velocity and acceleration (e.g., \texttt{mouthFrownRight\_vel\_max\_abs}) demonstrate that expression transitions---not just static poses---are crucial for affective recognition.

    \item \textbf{Arousal level:} The \texttt{arousal\_level} composite feature has the highest XGBoost importance (rank 1) but lower permutation importance (rank 103), suggesting it captures variance redundant with other features.
\end{enumerate}
...
30 & 2.00s & 0.631 & 0.411 & 957 & 2,152 \\
50 & 3.33s & 0.624 & 0.411 & 867 & 2,152 \\
75 & 5.00s & 0.623 & 0.416 & 869 & 2,152 \\
100 & 6.67s & 0.624 & \textbf{0.417} & 851 & 2,152 \\
